...
export function adjacentLiberties(x, y) {
...
export function identifyGroups(startX, startY, identifyOpposite = false) {
	// Determine the color of the group to identify: same as the starting point or opposite.
	const initialColor = rulesControl.getCellValue(startX, startY);
	...
	const exploreGroup = (x, y) => {
		...
		const currentColor = rulesControl.getCellValue(x, y);
	...
	startPoints.forEach(([x, y]) => {
		if (isValidCoordinate(x, y) && rulesControl.getCellValue(x, y) === targetColor && !visited.has(`${x},${y}`)) {
